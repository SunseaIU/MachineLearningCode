...
def labelPropagation(Mat_Label, Mat_Unlabel, labels, kernel_type='rbf', rbf_sigma=1.5,
    ...
    while iter < max_iter and changed > tol:
        if iter % 1 == 0:
            print(f"---> Iteration {iter}/{max_iter}, changed: {changed}")
...
def show(Mat_Label, labels, Mat_Unlabel, unlabel_data_labels):
    ...
    for i in range(Mat_Label.shape[0]):
        if int(labels[i]) == 0:
            plt.plot(Mat_Label[i, 0], Mat_Label[i, 1], 'Dr')  # 红色菱形
        elif int(labels[i]) == 1:
            plt.plot(Mat_Label[i, 0], Mat_Label[i, 1], '^b')  # 蓝色三角形 (修改处)
        else:
            plt.plot(Mat_Label[i, 0], Mat_Label[i, 1], 'Dy')  # 黄色菱形
    # 绘制未标记数据 (预测结果)
    for i in range(Mat_Unlabel.shape[0]):
        if int(unlabel_data_labels[i]) == 0:
            plt.plot(Mat_Unlabel[i, 0], Mat_Unlabel[i, 1], 'or')  # 红色圆形
        elif int(unlabel_data_labels[i]) == 1:
            plt.plot(Mat_Unlabel[i, 0], Mat_Unlabel[i, 1], '^b')  # 蓝色三角形 (修改处)
        else:
            plt.plot(Mat_Unlabel[i, 0], Mat_Unlabel[i, 1], 'oy')  # 黄色圆形
    plt.xlabel('X1')
    plt.ylabel('X2')
    plt.xlim(0.0, 12.0)
    plt.ylim(0.0, 12.0)
...
if __name__ == "__main__":
    ...
    unlabel_data_labels = labelPropagation(Mat_Label, Mat_Unlabel, labels, kernel_type='knn', knn_num_neighbors=10, max_iter=1)
